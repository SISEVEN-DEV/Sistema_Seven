...
namespace Seven_Sistema
{
    ...
    {
        public FrmPesqFormaPagamento(byte formulario, string usuario, string cod_pdv_computador)
        {
            InitializeComponent();
        ...
        private void FrmPesqFormaPagamento_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (_Formulario == 2)
                {
                    cbbpTipoPagamento.Items.Clear();
                    cbbpTipoPagamento.Items.Add("");
                    cbbpTipoPagamento.Items.Add("DINHEIRO");
                    cbbpTipoPagamento.Items.Add("CARTAO DE CREDITO");
                    cbbpTipoPagamento.Items.Add("CARTAO DE DEBITO");
                    cbbpTipoPagamento.Items.Add("PIX");
                    cbbpTipoPagamento.Items.Add("CREDITO LOJA");
                    cbbpTipoPagamento.Items.Add("CHEQUE");
                }
                //
        ...
        private void btnPesquisar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (bllFormaPagamento.Sel_Forma_Pagar_Todos(tipo_operacao, false) == null)
                    {
                    ...
                    {
                        dtFormaPagamento.DataSource = bllFormaPagamento.Sel_Forma_Pagar_Todos(tipo_operacao, false);
                ...
                else if (rbtnDescricao.Checked == true)
                {
                    ...
                    {
                        if (bllFormaPagamento.Sel_Forma_Pagar_Descricao(txtpDescricao.Text, tipo_operacao, false) == null)
                        {
                            dtFormaPagamento.DataSource = null;
                            MessageBox.Show("Nenhum registro encontrado.", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                            this.DialogResult = DialogResult.None;
                        ...
                        {
                            dtFormaPagamento.DataSource = bllFormaPagamento.Sel_Forma_Pagar_Descricao(txtpDescricao.Text, tipo_operacao, false);
                            dtFormaPagamento.Select();
                        }
                    }
                }
                else if (rbtnCodigo.Checked == true)
                {
                    if (txtpCodigo.Text != "")
                    {
                        if (bllFormaPagamento.Sel_Forma_Pagar_Codigo(txtpCodigo.Text, tipo_operacao, false) == null)
                        {
                            dtFormaPagamento.DataSource = null;
                            MessageBox.Show("Nenhum registro encontrado.", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                            this.DialogResult = DialogResult.None;
                        ...
                        {
                            dtFormaPagamento.DataSource = bllFormaPagamento.Sel_Forma_Pagar_Codigo(txtpCodigo.Text, tipo_operacao, false);
                            dtFormaPagamento.Select();
                        }
                    }
                }
                else if (rbtnTipo.Checked == true)
                {
                    ...
                    {
                        if (bllFormaPagamento.Sel_Forma_Tipo_Pagamento(cbbpTipoPagamento.Text, tipo_operacao, false) == null)
                        {
                            dtFormaPagamento.DataSource = null;
                            MessageBox.Show("Nenhum registro encontrado.", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                            this.DialogResult = DialogResult.None;
                        ...
                        {
                            dtFormaPagamento.DataSource = bllFormaPagamento.Sel_Forma_Tipo_Pagamento(cbbpTipoPagamento.Text, tipo_operacao, false);
                            dtFormaPagamento.Select();
                        }
                    }
                }
        ...
        private void btnIncluir_Click(object sender, EventArgs e)
        {
            try
            {
                if (_Formulario == 0)
                {
                    ...
                    {
                        MessageBox.Show("Não é possível selecionar a Forma de Pagamento\n[ NOTA PROMISSORIA ] para baixa/pagamento de uma Conta a Pagar.", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        this.DialogResult = DialogResult.None;
                ...
                else if (_Formulario == 1)
                {
                    ...
                    if (SelectedRow.Cells[2].Value.ToString() == "NOTA PROMISSORIA")
                    {
                        MessageBox.Show("Não é possível selecionar a Forma de Pagamento [ NOTA PROMISSORIA ] para baixa/pagamento de uma Conta a Receber.", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        this.DialogResult = DialogResult.None;
                ...
                else if (_Formulario == 2)
                {
                    ...
                    if (SelectedRow.Cells[2].Value.ToString() == "NOTA PROMISSORIA")
                    {
                        MessageBox.Show("Não é possível selecionar a Forma de Pagamento [ NOTA PROMISSÓRIA ] para baixa/pagamento de uma Conta a Receber.", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        this.DialogResult = DialogResult.None;
                    ...
                    {
                        btnIncluir.Select();
                        bllDFe._Forma_Pagamento_PesqFormaPagamento_Tabela = SelectedRow.Cells[0].Value.ToString() + "—" + SelectedRow.Cells[2].Value.ToString();
                        this.DialogResult = DialogResult.OK;
                    }
                }
                else if (_Formulario == 3)
                {
                    ...
                    if (SelectedRow.Cells[2].Value.ToString() == "NOTA PROMISSORIA")
                    {
                        MessageBox.Show("Não é possível selecionar a Forma de Pagamento [ NOTA PROMISSORIA ] para baixa/pagamento de uma Ordem de Serviço.", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        this.DialogResult = DialogResult.None;
                    ...
                    {
                        btnIncluir.Select();
                        bllOS._Forma_Pagamento_PesqFormaPagamento_Tabela = SelectedRow.Cells[0].Value.ToString() + "—" + SelectedRow.Cells[2].Value.ToString();
                        this.DialogResult = DialogResult.OK;
                    }
                }
                else if (_Formulario == 4)
                {
                    ...
                    //
                    btnIncluir.Select();
                    bllVenda._Pagamento_PesqPagamento_Tabela = SelectedRow.Cells[0].Value.ToString() + "—" + SelectedRow.Cells[2].Value.ToString();
                    this.DialogResult = DialogResult.OK;
                }
        ...
        private void dtFormaPagamento_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (_Formulario == 0)
                {
                    ...
                    if (SelectedRow.Cells[2].Value.ToString() == "NOTA PROMISSORIA")
                    {
                        MessageBox.Show("Não é possível selecionar a Forma de Pagamento [ NOTA PROMISSORIA ] para baixa/pagamento de uma Conta a Pagar.", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        this.DialogResult = DialogResult.None;
                ...
                else if (_Formulario == 3)
                {
                    ...
                    if (SelectedRow.Cells[2].Value.ToString() == "NOTA PROMISSORIA")
                    {
                        MessageBox.Show("Não é possível selecionar a Forma de Pagamento [ NOTA PROMISSÓRIA ] para baixa/pagamento de uma Ordem de Serviço.", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        this.DialogResult = DialogResult.None;
            ...
            if (e.KeyCode == Keys.Enter)
            {
                try
                {
                    if (_Formulario == 0)
                    {
                        ...
                        if (SelectedRow.Cells[2].Value.ToString() == "NOTA PROMISSORIA")
                        {
                            MessageBox.Show("Não é possível selecionar a Forma de Pagamento [ NOTA PROMISSÓRIA ] para baixa/pagamento de uma Conta a Pagar.", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                            this.DialogResult = DialogResult.None;
                        }
                        else
                        {
        ...
        private void FrmPesqFormaPagamento_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                using (StreamWriter writer = new StreamWriter(@"C:\Sistema SEVEN\Config\Log\Log de Acoes\" + DateTime.Now.ToShortDateString().Replace('/', '_') + ".txt", true))
                {
                    writer.WriteLine(DateTime.Now.ToShortTimeString() + " - Formulário FrmPesqFormaPagamento foi finalizado.");
                }
                using (StreamWriter writer = new StreamWriter(@"C:\Windows\Temp\Sistema SEVEN\Log de Acoes\" + DateTime.Now.ToShortDateString().Replace('/', '_') + ".txt", true))
                {
                    writer.WriteLine(DateTime.Now.ToShortTimeString() + " - Formulário FrmPesqFormaPagamento foi finalizado.");
        ...
        private void btnCadastrar_Click(object sender, EventArgs e)
        {
            try
            {
                if (bllUsuario.Sel_Cadastrar_Forma_Pagamento(_Usuario) == true)
                {
                    using (FrmCadFormaPagamento Pag = new FrmCadFormaPagamento(_Usuario, _Cod_PDV_Computador, 1))
                    {
                        if (Pag.ShowDialog() == DialogResult.OK)
                        {
                            ...
                            //
                            if (bllFormaPagamento.Sel_Forma_Pagar_Codigo(bllFormaPagamento._Cod_Forma_Pagamento_Cadastro, tipo_operacao, false) == null)
                            {
                                dtFormaPagamento.DataSource = null;
                            }
                            else
                            {
                                dtFormaPagamento.DataSource = bllFormaPagamento.Sel_Forma_Pagar_Codigo(bllFormaPagamento._Cod_Forma_Pagamento_Cadastro, tipo_operacao, false);
                                dtFormaPagamento.Select();
                            }
                        }
                        else
