...
namespace Seven_Sistema
{
    public partial class FrmCapturarOrcamento : Form
    {
        ...
        public string _Cod_PDV_Computador;
        ...
        {
            MessageBox.Show("Informe o código do orçamento e clique em salvar para capturar.", "Informação", MessageBoxButtons.OK, MessageBoxIcon.Information);
            this.DialogResult = DialogResult.None;
        }
        ...
        private void btnSalvar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        MessageBox.Show("O orçamento de código: [ " + txtCodigo.Text + " ] está expirado. Deseja continuar?", "Pergunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                        if (DialogResult == DialogResult.Yes)
                        {
                            MessageBox.Show("Orçamento encontrado com sucesso.", "Informação", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            bllDFe._Tipo_Captura = 0;
                            bllDFe._Cod_Orcamento = txtCodigo.Text;
                    ...
                    else
                    {
                        if (_Formulario == 0)
                        {
                            ...
                            {
                                DialogResult = MessageBox.Show("Este Orçamento já foi realizado. Deseja continuar?", "Pergunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                                if (DialogResult == DialogResult.Yes)
                                {
                                    MessageBox.Show("Orçamento encontrado com sucesso.", "Informação", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                    bllDFe._Tipo_Captura = 0;
                        ...
                        else if (_Formulario == 1)
                        {
                            ...
                            {
                                MessageBox.Show("Orçamento encontrado com sucesso.", "Informação", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                bllVenda._Cod_Orcamento_Orc = txtCodigo.Text;
                                this.DialogResult = DialogResult.OK;
                            }
                            else if (dr["situacao"].ToString() == "REALIZADO")
                            {
                        ...
                        else if (_Formulario == 2)
                        {
                            ...
                            {
                                MessageBox.Show("Orçamento encontrado com sucesso.", "Informação", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                bllOS._Cod_Orcamento_Cadastro = txtCodigo.Text;
                                this.DialogResult = DialogResult.OK;
                            }
                            else if (dr["situacao"].ToString() == "REALIZADO")
                            {
                                DialogResult = MessageBox.Show("Este Orçamento já foi realizado.", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                this.DialogResult = DialogResult.None;
                                txtCodigo.Select();
                            }
                        }
            ...
            catch (Exception ex)
            {
                ...
                bllVenda._Cod_Orcamento_Orc = null;
                txtCodigo.Text = null;
                txtCodigo.Select();
            }
        }
        ...
        {
            this.Enabled = false;
            using (FrmPesqOrcamento Orc = new FrmPesqOrcamento(0, _Usuario, _Cod_PDV_Computador))
            {
        ...
        private void txtCodigo_QueryContinueDrag(object sender, QueryContinueDragEventArgs e)
        {
