...
{
    public partial class FrmPesqEntidadeBancaria : Form
    {
        public FrmPesqEntidadeBancaria(byte formulario)
        {
            InitializeComponent();
        ...
        private void FrmPesqEntidadeBancaria_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    writer.WriteLine(DateTime.Now.ToShortTimeString() + " - Formulário FrmPesqEntidadeBancaria iniciado.");
                }
            ...
            catch (Exception ex)
            {
                ...
                using (StreamWriter writer = new StreamWriter(@"C:\Sistema SEVEN\Config\Log\Log de Acoes\" + DateTime.Now.ToShortDateString().Replace('/', '_') + ".txt", true))
                {
                    writer.WriteLine(DateTime.Now.ToShortTimeString() + " - Erro: " + ex.Message + " | Evento load do formulário FrmPesqEntidadeBancaria.");
                }
                using (StreamWriter writer = new StreamWriter(@"C:\Windows\Temp\Sistema SEVEN\Log de Acoes\" + DateTime.Now.ToShortDateString().Replace('/', '_') + ".txt", true))
                {
                    writer.WriteLine(DateTime.Now.ToShortTimeString() + " - Erro: " + ex.Message + " | Evento load do formulário FrmPesqEntidadeBancaria.");
        ...
        private void dtPesquisa_RowsAdded(object sender, DataGridViewRowsAddedEventArgs e)
        {
            ...
            dtPesquisa.Columns[1].HeaderText = "Código COMPE";
            dtPesquisa.Columns[2].HeaderText = "Descrição";
            dtPesquisa.Columns[3].HeaderText = "Palavra-Chave";
            dtPesquisa.Columns[4].Visible = false;

            dtPesquisa.Columns[0].Width = 95;
            dtPesquisa.Columns[1].Width = 125;
            dtPesquisa.Columns[2].Width = 397;
        ...
        private void btnPesquisar_Click(object sender, EventArgs e)
        {
            try
            {
                if (rbtnDescricao.Checked == true)
                {
                    if (txtpNome.Text != "")
                    {
                        if (bllEntidadeBancaria.Sel_Entidade_Bancaria_Descricao(txtpNome.Text) == null)
                        {
                        ...
                        {
                            dtPesquisa.DataSource = bllEntidadeBancaria.Sel_Entidade_Bancaria_Descricao(txtpNome.Text);
                            dtPesquisa.Select();
                        }
                    }
                }
                else if (rbtnCodigo.Checked == true)
                {
                    if (txtpCodigo.Text != "")
                    {
                        ...
                        {
                            dtPesquisa.DataSource = bllEntidadeBancaria.Sel_Entidade_Bancaria_Codigo(txtpCodigo.Text);
                            dtPesquisa.Select();
                        }
                    }
                }
                else if (rbtnPalavraChave.Checked == true)
                {
                    if (txtpPalavraChave.Text != "")
                    {
                        ...
                        {
                            dtPesquisa.DataSource = bllEntidadeBancaria.Sel_Entidade_Bancaria_Palavra_Chave(txtpPalavraChave.Text);
                            dtPesquisa.Select();
                        }
                    }
                }
                else if (rbtnCodigoCompe.Checked == true)
                {
                    if (txtpCodigo.Text != "")
                    {
                        ...
                        {
                            dtPesquisa.DataSource = bllEntidadeBancaria.Sel_Entidade_Bancaria_Codigo_Compe(txtpCodigo.Text);
                            dtPesquisa.Select();
                        }
                    }
                }
                else if (rbtnTodos.Checked == true)
                {
                    ...
                    {
                        dtPesquisa.DataSource = bllEntidadeBancaria.Sel_Entidade_Bancaria_Todos();
                        dtPesquisa.Select();
                    }
                }
        ...
        private void FrmPesqEntidadeBancaria_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void FrmPesqEntidadeBancaria_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                using (StreamWriter writer = new StreamWriter(@"C:\Sistema SEVEN\Config\Log\Log de Acoes\" + DateTime.Now.ToShortDateString().Replace('/', '_') + ".txt", true))
                {
                    writer.WriteLine(DateTime.Now.ToShortTimeString() + " - Formulário FrmPesqEntidadeBancaria foi finalizado.");
                }
                using (StreamWriter writer = new StreamWriter(@"C:\Windows\Temp\Sistema SEVEN\Log de Acoes\" + DateTime.Now.ToShortDateString().Replace('/', '_') + ".txt", true))
                {
                    writer.WriteLine(DateTime.Now.ToShortTimeString() + " - Formulário FrmPesqEntidadeBancaria foi finalizado.");
            ...
            catch (Exception ex)
            {
                ...
                using (StreamWriter writer = new StreamWriter(@"C:\Sistema SEVEN\Config\Log\Log de Acoes\" + DateTime.Now.ToShortDateString().Replace('/', '_') + ".txt", true))
                {
                    writer.WriteLine(DateTime.Now.ToShortTimeString() + " - Erro: " + ex.Message + " | Evento formclosing do formulário FrmPesqEntidadeBancaria.");
                }
                using (StreamWriter writer = new StreamWriter(@"C:\Windows\Temp\Sistema SEVEN\Log de Acoes\" + DateTime.Now.ToShortDateString().Replace('/', '_') + ".txt", true))
                {
                    writer.WriteLine(DateTime.Now.ToShortTimeString() + " - Erro: " + ex.Message + " | Evento formclosing do formulário FrmPesqEntidadeBancaria.");
        ...
        private void dtPesquisa_DoubleClick(object sender, EventArgs e)
        {
            if (_Formulario == 0)
            {
                ...
                bllContasPagar._Entidade_Bancaria_PesqEntidadeBancaria_Tabela = SelectedRow.Cells[0].Value.ToString() + "—" + SelectedRow.Cells[2].Value.ToString();
                this.DialogResult = DialogResult.OK;
            }
            else if (_Formulario == 1)
            {
                DataGridViewRow SelectedRow = dtPesquisa.Rows[dtPesquisa.CurrentRow.Index];
                bllFormaPagamento._Entidade_Bancaria_PesqEntidadeBancaria_Tabela = SelectedRow.Cells[0].Value.ToString() + "—" + SelectedRow.Cells[2].Value.ToString();
                this.DialogResult = DialogResult.OK;
            }
            else if (_Formulario == 2)
            {
                btnIncluir.Select();
                DataGridViewRow SelectedRow = dtPesquisa.Rows[dtPesquisa.CurrentRow.Index];
                bllControleCheque._Entidade_Bancaria_PesqControleCheque = SelectedRow.Cells[0].Value.ToString() + "—" + SelectedRow.Cells[2].Value.ToString();
                this.DialogResult = DialogResult.OK;
            }
        }
        ...
        private void rbtnCodigoCompe_CheckedChanged(object sender, EventArgs e)
        {
            ...
            lblPesquisar.Location = new Point(409, 19);
            txtpCodigo.Visible = true;
            txtpNome.Visible = false;
            txtpCodigo.Text = null;
            txtpCodigo.Select();
