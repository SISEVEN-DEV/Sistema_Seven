...
{
    public partial class FrmDataInventario : Form
    {
        public FrmDataInventario()
        {
            InitializeComponent();
        }
        ...
        private void FrmDataInventario_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    writer.WriteLine(DateTime.Now.ToShortTimeString() + " - Formulário FrmDataInventario iniciado.");
                }
                ...
                {
                    mtxtUltimoInventario.Text = bllInventario.Sel_Ultima_Data_Inventario_Adicionado().Remove(10);
                }
                else
                {
                    mtxtUltimoInventario.Enabled = false;
                    label2.Enabled = false;
                }
                //
                if (bllInventario.Sel_Localizacao_Inv() == null)
                {
                ...
                else
                {
                    ...
                    foreach (DataRow dr in bllInventario.Sel_Localizacao_Inv().Rows)
                    {
            ...
            catch (Exception ex)
            {
                ...
                using (StreamWriter writer = new StreamWriter(@"C:\Sistema SEVEN\Config\Log\Log de Acoes\" + DateTime.Now.ToShortDateString().Replace('/', '_') + ".txt", true))
                {
                    writer.WriteLine(DateTime.Now.ToShortTimeString() + " - Erro: " + ex.Message + " | Evento load do formulário FrmDataInventario.");
                }
                using (StreamWriter writer = new StreamWriter(@"C:\Windows\Temp\Sistema SEVEN\Log de Acoes\" + DateTime.Now.ToShortDateString().Replace('/', '_') + ".txt", true))
                {
                    writer.WriteLine(DateTime.Now.ToShortTimeString() + " - Erro: " + ex.Message + " | Evento load do formulário FrmDataInventario.");
        ...
        private void btnContinuar_Click(object sender, EventArgs e)
        {
            ...
            mtxtUltimoInventario.TextMaskFormat = MaskFormat.ExcludePromptAndLiterals;

            if (mtxtpData.Text == "" || mtxtpData1.Text == "")
            {
                MessageBox.Show("Existem campos obrigatórios que precisam ser preenchidos:\n< Data de Inicio e Final do Inventário >.", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                ...
                mtxtUltimoInventario.TextMaskFormat = MaskFormat.IncludePromptAndLiterals;
            }
            else
            {
                mtxtpData.TextMaskFormat = MaskFormat.IncludePromptAndLiterals;
                ...
                mtxtUltimoInventario.TextMaskFormat = MaskFormat.IncludePromptAndLiterals;
                //
                bllInventario.Salvar(mtxtpData.Text, mtxtpData1.Text, txtDescricao.Text, cbbLocalizacao.Text, chkbInvCont.Checked);
        ...
        {
            this.Enabled = false;
            using (FrmDatePicker2 Data = new FrmDatePicker2(25))
            {
                if (Data.ShowDialog() == DialogResult.OK)
                {
                    mtxtpData.Text = bllInventario._Data_DatePicker1;
                    mtxtpData1.Text = bllInventario._Data_DatePicker2;
                }
            }
            this.Enabled = true;
        }
        ...
        private void FrmDataInventario_KeyUp(object sender, KeyEventArgs e)
        {
        ...
        private void FrmDataInventario_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                using (StreamWriter writer = new StreamWriter(@"C:\Sistema SEVEN\Config\Log\Log de Acoes\" + DateTime.Now.ToShortDateString().Replace('/', '_') + ".txt", true))
                {
                    writer.WriteLine(DateTime.Now.ToShortTimeString() + " - Formulário FrmDataInventario foi finalizado.");
                }
                using (StreamWriter writer = new StreamWriter(@"C:\Windows\Temp\Sistema SEVEN\Log de Acoes\" + DateTime.Now.ToShortDateString().Replace('/', '_') + ".txt", true))
                {
                    writer.WriteLine(DateTime.Now.ToShortTimeString() + " - Formulário FrmDataInventario foi finalizado.");
        ...
        private void mtxtUltimoInventario_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == 13)
            {
                mtxtpData.Select();
        ...
        {
            this.Enabled = false;
            using (FrmPesqLocalizacao Localizacao = new FrmPesqLocalizacao(1))
            {
                if (Localizacao.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        ...
                        //
                        cbbLocalizacao.Text = bllInventario._Localizacao_PesqLocalizacao_Tabela;
                        bllInventario._Localizacao_PesqLocalizacao_Tabela = null;
                        cbbLocalizacao.Select();
                    }
                    catch (Exception ex)
                    {
        ...
        {
            MessageBox.Show("1 - Último Inventário: Informação do último valor de data recente de um inventário já realizado.", "Informação", MessageBoxButtons.OK, MessageBoxIcon.Information);
            this.DialogResult = DialogResult.None;
        }

        private void chkbInvCont_MouseMove(object sender, MouseEventArgs e)
        {
            this.Cursor = Cursors.Hand;
        }

        private void chkbInvCont_MouseLeave(object sender, EventArgs e)
        {
            this.Cursor = Cursors.Default;
        }
